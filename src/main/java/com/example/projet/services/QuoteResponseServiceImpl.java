...
public class QuoteResponseServiceImpl implements IQuoteResponseService {
    ...
    public QuoteResponse respond(Long quoteRequestId, Long clinicId, QuoteResponse response){
        ...
        QuoteRequest quoteRequest = quoteRequestRepository.findById(quoteRequestId).orElseThrow(() -> new RuntimeException("Quote request not found with id: " + quoteRequestId));
        Clinic clinic = clinicRepository.findById(clinicId).orElseThrow(() -> new RuntimeException("Clinic not found with id: " + clinicId));
